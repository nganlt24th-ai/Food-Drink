...
namespace Web.Areas.Admin.Controllers
{
    ...
    public class MenuController : Controller
    {
        ...
        public MenuController(FoodContext dbContext, IWebHostEnvironment environment)
        {
        ...
        {
            var items = (from i in _dbContext.Menus select i);
            int recordsTotal = 0;
        ...
        public async Task<IActionResult> getItem(Guid id)
        {
            if (_dbContext.Menus == null)
                return NotFound();
            var item = await _dbContext.Menus.FindAsync(id);
            if (item == null)
                return NotFound();
            return Ok(item);
        }
        [HttpPost]
        public async Task<IActionResult> Save(MenuViewModel model, IFormFile Picture)
        {
            Core.Database.Models.Menu item;
            var loggedMember = HttpContext.Session.GetObject<Member>("member");
            if (model.Id == null)
            {
                item = new Core.Database.Models.Menu();
            ...
            {
                item = await _dbContext.Menus.FindAsync(model.Id);
            ...
            if (Picture != null)
            {
                var path = Path.Combine(this._environment.WebRootPath, "img/menus/", Picture.FileName);
                using (FileStream stream = new FileStream(path, FileMode.Create))
                {
                    await Picture.CopyToAsync(stream);
                    stream.Close();
                }
                item.Picture = "/img/menus/" + Picture.FileName;
            }
