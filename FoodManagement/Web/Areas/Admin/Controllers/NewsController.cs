...
namespace Web.Areas.Admin.Controllers
{
    ...
    public class NewsController : Controller
    {
        ...
        {
            var items = (from i in _dbContext.News select i);
            int recordsTotal = 0;
        ...
        {
            Core.Database.Models.New item;
            var loggedMember = HttpContext.Session.GetObject<Member>("member");
            if (model.Id == null)
            {
                item = new Core.Database.Models.New();
                item.Id = Guid.NewGuid();
                item.CreatedBy = loggedMember.Id;
                item.CreatedOn = DateTime.Now;
                await _dbContext.News.AddAsync(item);
            }
            else
            {
                item = await _dbContext.News.FindAsync(model.Id);
